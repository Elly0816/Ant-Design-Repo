...
function reduceCountries(countries: COUNTRY[]): SelectProps[] {
  ...
  SelectProps
*/
    const newItem: SelectProps[] = countries.reduce(
        (arr: SelectProps[], country: COUNTRY) => {
...
export default function App(): ReactElement {
    ...
    const countriesToShow = useMemo(
        () => reduceCountries(countries),
        [countries]
    )
    const [defValue, setDefValue] = useState<{
        label: string
        value: string | undefined
    }>({ label: countriesToShow[0].value, value: countriesToShow[0].id })
    ...
    useEffect(() => {
        ...
        if (
            !oldData ||
            (oldData && !((symbol as string) in JSON.parse(oldData)))
        ) {
            getData(symbol as string)
        } else {
            //Ensures that the base currency does not appear in the list of rates
            setData(
                JSON.parse(oldData as string)[symbol as string].filter(
                    (item: DataType) =>
                        item.name.currencyCode !== defValue.value
                )
            )
    ...
    const getData = useCallback(
        (symbol: string) => {
            ...
            getTableData(symbol as string, signal).then((res: tableData[]) => {
                ...
                setData(
                    newData.filter(
                        (item) => item.name.currencyCode !== defValue.value
                    )
                )
                localStorage.setItem(
                    'data',
                    JSON.stringify({ [symbol as string]: newData })
                )
                setLoading(false)
            })
        },
  ...
  */
        let favsFromStorage: string | null | { codes: string } =
